...
public class CRPVouchersHtmlForPDF
{
    #region Variables
    ...
    public string GetHtmlForPDF(DataSet dsDetails)
    {
        try
        {
            if (dsDetails.Tables.Count > 0 && dsDetails.Tables[0].Rows.Count > 0)
            {
                ...
                //if (dr0["BILL_NO"] != DBNull.Value) BillNo = Convert.ToString(dr0["BILL_NO"]).Trim();
                ...
                if (dr0["NET_AMOUNT"] != DBNull.Value) NetAmount = Convert.ToString(dr0["NET_AMOUNT"]).Trim();
                if (dr0["CURR_AMOUNT"] != DBNull.Value) CurrAmount = Convert.ToString(dr0["CURR_AMOUNT"]).Trim();
            ...
            sb.Append("<h2 class='headerStyle'><u>CUSTOMER RECEIPT PAYMENT VOUCHER COPY</u></h2>\n");
            sb.Append("<hr />\n");
            ...
            sb.Append("<tr>\n");
            sb.Append("<td class='td1header'>Bill No.:</td>\n");
            sb.Append("<td class='td2header'><b>{#BillNo#}</b></td>\n");
            sb.Append("</tr>\n");
            ...
            sb.Append("<td class='td1header'>Curr Amount:</td>\n");
            sb.Append("<td class='td2header'>{#CurrAmount#}</td>\n");
            sb.Append("</tr>\n");
            ...
            sb.Replace("{#CurrAmount#}", CurrAmount);
            ...
            if (dsDetails.Tables[1].Rows.Count > 0)
            {
                ...
                sb.Append("<th class='tdtag'>Bill No.</th>\n");
                sb.Append("<th class='tdquantity'>Net Amount</th>\n");
                sb.Append("<th class='tdquantity'>Curr Amount</th>\n");
                ...
                foreach (DataRow dr in dsDetails.Tables[1].Rows)
                {
                    ...
                    sb.Append("<td class='tdtag'>{#BillNo#}</td>\n");
                    sb.Append("<td class='tdquantity'>{#NetAmount#}</td>\n");
                    sb.Append("<td class='tdquantity'>{#CurrAmount#}</td>\n");

                    sb.Append("</tr>\n");

                    sb.Replace("{#SRNo#}", Convert.ToString(srNo));
                    sb.Replace("{#BillNo#}", Convert.ToString(dr["BILL_NO"]));
                    sb.Replace("{#NetAmount#}", Convert.ToString(dr["NET_AMOUNT"]));
                    sb.Replace("{#CurrAmount#}", Convert.ToString(dr["CURR_AMOUNT"]));
