...
public class IRVouchersHtmlForPDF
{
    #region Variables
    ...
    string ReferenceVoucherNo = string.Empty;
    string ReferenceVoucherDate = string.Empty;
    ...
    private string Factor = string.Empty;
    ...
    public string GetHtmlForPDF(DataSet dsDetails)
    {
        try
        {
            if (dsDetails.Tables.Count > 0 && dsDetails.Tables[0].Rows.Count > 0)
            {
                ...
                if(dr0["REFERENCE_VOUCHER_NO"] != DBNull.Value) ReferenceVoucherNo = Convert.ToString(dr0["REFERENCE_VOUCHER_NO"]).Trim();
                if (dr0["REFERENCE_VOUCHER_DATE"] != DBNull.Value) ReferenceVoucherDate = Convert.ToString(dr0["REFERENCE_VOUCHER_DATE"]).Trim();
            ...
            sb.Append("<h2 class='headerStyle'><u>INVENTORY RETURN VOUCHER COPY</u></h2>\n");
            sb.Append("<hr />\n");
            ...
            sb.Append("<tr>\n");
            sb.Append("<td class='td1header'>Reference Voucher No.:</td>\n");
            sb.Append("<td class='td2header'><b>{#ReferenceVoucherNo#}</b></td>\n");
            sb.Append("<td class='td1header'>Reference Voucher Date:</td>\n");
            sb.Append("<td class='td2header'>{#ReferenceVoucherDate#}</td>\n");
            sb.Append("</tr>\n");
            ...
            sb.Replace("{#ReferenceVoucherNo#}", ReferenceVoucherNo);
            sb.Replace("{#ReferenceVoucherDate#}", ReferenceVoucherDate);
            ...
            if (dsDetails.Tables[1].Rows.Count > 0)
            {
                ...
                sb.Append("<th class='tdtag'>DOC Class</th>\n");
                sb.Append("<th class='tdtag'>Product Code</th>\n");
                sb.Append("<th class='tddesc'>Product Description</th>\n");

                sb.Append("<th class='tdtag'>Returned Product Code</th>\n");
                sb.Append("<th class='tddesc'>Returned Product Description</th>\n");

                sb.Append("<th class='tdtag'>Cost Center Code</th>\n");
                sb.Append("<th class='tddesc'>Cost Center Name</th>\n");
                ...
                foreach (DataRow dr in dsDetails.Tables[1].Rows)
                {
                    ...
                    sb.Append("<td class='tdtag'>{#DOCClass#}</td>\n");
                    sb.Append("<td class='tddesc'>{#ProductCode#}</td>\n");
                    sb.Append("<td class='tdtag'>{#ProductDescription#}</td>\n");

                    sb.Append("<td class='tddesc'>{#ReturnedProductCode#}</td>\n");
                    sb.Append("<td class='tdtag'>{#ReturnedProductDescription#}</td>\n");

                    sb.Append("<td class='tdtag'>{#CostCenterCode#}</td>\n");
                    sb.Append("<td class='tddesc'>{#CostCenterName#}</td>\n");
                    ...
                    sb.Replace("{#DOCClass#}", Convert.ToString(dr["DOC_CLASS"]));
                    ...
                    sb.Replace("{#ReturnedProductCode#}", Convert.ToString(dr["RETURNED_PRODUCT_CODE"]));
                    sb.Replace("{#ReturnedProductDescription#}", Convert.ToString(dr["RETURNED_PRODUCT_DESCRIPTION"]));
                    
                    sb.Replace("{#CostCenterCode#}", Convert.ToString(dr["COST_CENTER_CODE"]));
                    sb.Replace("{#CostCenterName#}", Convert.ToString(dr["COST_CENTER_NAME"]));
