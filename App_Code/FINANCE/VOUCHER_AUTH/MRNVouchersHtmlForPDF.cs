...
public class MRNVouchersHtmlForPDF
{
    #region Variables
    ...
    public string GetHtmlForPDF(DataSet dsDetails)
    {
        try
        {
            if (dsDetails.Tables.Count > 0 && dsDetails.Tables[0].Rows.Count > 0)
            {
                ...
                if (dr0["GATE_ENTRY_NO"] != DBNull.Value) GateEntryNo = Convert.ToString(dr0["GATE_ENTRY_NO"]).Trim();
                if (dr0["GATE_ENTRY_DATE"] != DBNull.Value) GateEntryDate = Convert.ToString(dr0["GATE_ENTRY_DATE"]).Trim();
            ...
            sb.Append("<h2 class='headerStyle'><u>MRN VOUCHER COPY</u></h2>\n");
            sb.Append("<hr />\n");
            ...
            sb.Append("<td class='td2header'><b>{#PurchaseOrderDate#}</b></td>\n");
            sb.Append("<td class='td1header'>Vendor Code:</td>\n");
            sb.Append("<td class='td2header'>{#VendorCode#}</td>\n");
            sb.Append("</tr>\n");
            ...
            sb.Append("<tr>\n");
            sb.Append("<td class='td1header'>Gate Entry No.:</td>\n");
            sb.Append("<td class='td2header'><b>{#GateEntryNo#}</b></td>\n");
            sb.Append("<td class='td1header'>Gate Entry Date:</td>\n");
            sb.Append("<td class='td2header'>{#GateEntryDate#}</td>\n");
            sb.Append("</tr>\n");
            ...
            sb.Replace("{#GateEntryNo#}", GateEntryNo);
            sb.Replace("{#GateEntryDate#}", GateEntryDate);
            ...
            if (dsDetails.Tables[1].Rows.Count > 0)
            {
                ...
                sb.Append("<th class='tdcate'>Purchase Order No</th>\n");
                ...
                foreach (DataRow dr in dsDetails.Tables[1].Rows)
                {
                    ...
                    sb.Append("<td class='tdcate'>{#PurchaseOrderNo#}</td>\n");
                    ...
                    sb.Replace("{#PurchaseOrderNo#}", Convert.ToString(dr["PURCHASEE_ORDER_NO"]));
