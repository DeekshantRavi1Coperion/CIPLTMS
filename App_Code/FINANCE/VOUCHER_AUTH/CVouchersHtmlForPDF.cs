...
public class CVouchersHtmlForPDF
{
    #region Variables
    ...
    //private string ReceiptAmountFC;
    //private string PaymentAcountFC;
    //private string ReceiptAmountINR;
    //private string PaymentAcountINR;
    //private string PaidToOrReceivedFrom;
    private string Cheque;
    private string CheqheDated;
    //private string BankTransactionRefOrUTR;
    ...
    public string GetHtmlForPDF(DataSet dsDetails)
    {
        try
        {
            if (dsDetails.Tables.Count > 0 && dsDetails.Tables[0].Rows.Count > 0)
            {
                ...
                if (dr0["AMOUNT"] != DBNull.Value) Amount = Convert.ToString(dr0["AMOUNT"]).Trim();
                if (dr0["CURRENCY_DESC"] != DBNull.Value) CurrencyDesc = Convert.ToString(dr0["CURRENCY_DESC"]).Trim();
                if (dr0["NARRATION"] != DBNull.Value) Narration = Convert.ToString(dr0["NARRATION"]).Trim();
                ...
                if (dr0["CHEQUE_OR_REFERENCE_NO"] != DBNull.Value) Cheque = Convert.ToString(dr0["CHEQUE_OR_REFERENCE_NO"]).Trim();
                if (dr0["CHEQUE_DATED"] != DBNull.Value) CheqheDated = Convert.ToString(dr0["CHEQUE_DATED"]).Trim();
                ...
                //if (dr0["GLCODE"] != DBNull.Value) GlCode = Convert.ToString(dr0["GLCODE"]).Trim();
                //if (dr0["GL_DESCRIPTION"] != DBNull.Value) GlDescription = Convert.ToString(dr0["GL_DESCRIPTION"]).Trim();
                //if (dr0["RECEIPT_AMOUNT_FC"] != DBNull.Value) ReceiptAmountFC = Convert.ToString(dr0["RECEIPT_AMOUNT_FC"]).Trim();
                //if (dr0["PAYMENT_AMOUNT_FC"] != DBNull.Value) PaymentAcountFC = Convert.ToString(dr0["PAYMENT_AMOUNT_FC"]).Trim();
                //if (dr0["RECEIPT_AMOUNT_INR"] != DBNull.Value) ReceiptAmountINR = Convert.ToString(dr0["RECEIPT_AMOUNT_INR"]).Trim();
                //if (dr0["PAYMENT_AMOUNT_INR"] != DBNull.Value) PaymentAcountINR = Convert.ToString(dr0["PAYMENT_AMOUNT_INR"]).Trim();
                //if (dr0["PAID_TO_OR_RECEIVED_FROM"] != DBNull.Value) PaidToOrReceivedFrom = Convert.ToString(dr0["PAID_TO_OR_RECEIVED_FROM"]).Trim();
                //if (dr0["BANK_TRANSACTION_REF_NO_OR_UTR"] != DBNull.Value) BankTransactionRefOrUTR = Convert.ToString(dr0["BANK_TRANSACTION_REF_NO_OR_UTR"]).Trim();                
                //if (dr0["RECONCILED_DATE"] != DBNull.Value) ReconcileDate = Convert.ToString(dr0["RECONCILED_DATE"]).Trim();                
            ...
            sb.Append("<h2 class='headerStyle'><u>CONTRA VOUCHER COPY</u></h2>\n");
            sb.Append("<hr />\n");
            sb.Append("<table class='tblheader'>\n");

            sb.Append("<tr>\n");
            sb.Append("<td class='td1header'>Voucher No.:</td>\n");
            sb.Append("<td class='td2header'><b>{#VoucherNo#}</td>\n");
            sb.Append("<td class='td1header'>Voucher Date:</td>\n");
            sb.Append("<td class='td2header'>{#VoucherDate#}</td>\n");
            sb.Append("</tr>\n");

            sb.Append("<tr>\n");            
            sb.Append("<td class='td1header'>Amount:</td>\n");
            sb.Append("<td class='td2header'>{#Amount#}</td>\n");
            sb.Append("<td class='td1header'>Currency Desc:</td>\n");
            sb.Append("<td class='td2header'>{#CurrencyDesc#}</td>\n");
            sb.Append("</tr>\n");
                             
            sb.Append("<tr>\n");           
            sb.Append("<td class='td1header'>Cheque / Reference No.:</td>\n");
            sb.Append("<td class='td2header'>{#Cheque#}</td>\n");
            sb.Append("<td class='td1header'>Cheqhe Dated:</td>\n");
            sb.Append("<td class='td2header'>{#CheqheDated#}</td>\n");
            sb.Append("</tr>\n");
           
            sb.Append("<tr>\n");            
            sb.Append("<td class='td1header'>Narration:</td>\n");
            sb.Append("<td class='td2header'>{#Narration#}</td>\n");
            sb.Append("<td class='td1header'>Voucher Created By:</td>\n");
            sb.Append("<td class='td2header'>{#VoucherCreatedBy#}</td>\n");
            sb.Append("</tr>\n");
            ...
            sb.Replace("{#GlCode#}", GlCode);
            sb.Replace("{#GlDescription#}", GlDescription);
            sb.Replace("{#CurrencyDesc#}", CurrencyDesc);
            sb.Replace("{#Amount#}", Amount);
            //sb.Replace("{#ReceiptAmountFC#}", ReceiptAmountFC);
            //sb.Replace("{#PaymentAcountFC#}", PaymentAcountFC);
            //sb.Replace("{#ReceiptAmountINR#}", ReceiptAmountINR);
            //sb.Replace("{#PaymentAcountINR#}", PaymentAcountINR);
            //sb.Replace("{#PaidToOrReceivedFrom#}", PaidToOrReceivedFrom);
            sb.Replace("{#Cheque#}", Cheque);
            sb.Replace("{#CheqheDated#}", CheqheDated);
            //sb.Replace("{#BankTransactionRefOrUTR#}", BankTransactionRefOrUTR);
            //sb.Replace("{#ReconcileDate#}", ReconcileDate);
            sb.Replace("{#Narration#}", Narration);
            ...
            if (dsDetails.Tables[1].Rows.Count > 0)
            {
                ...
                sb.Append("<th class='tdtag'>Type</th>\n");
                sb.Append("<th class='tdquantity'>Debit Amount</th>\n");
                sb.Append("<th class='tdquantity'>Credit Amount</th>\n");
                ...
                foreach (DataRow dr in dsDetails.Tables[1].Rows)
                {
                    ...
                    sb.Append("<td class='tdtag'>{#Type#}</td>\n");
                    sb.Append("<td class='tdquantity'>{#DebitAmount#}</td>\n");
                    sb.Append("<td class='tdquantity'>{#CreditAmount#}</td>\n");
                    ...
                    sb.Replace("{#DebitAmount#}", Convert.ToString(dr["DEBIT_AMOUNT"]));
                    sb.Replace("{#CreditAmount#}", Convert.ToString(dr["CREDIT_AMOUNT"]));
                }
                sb.Append("</table>\n");
            }
