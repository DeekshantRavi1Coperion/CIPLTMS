<%@ Page Language="C#" MasterPageFile="~/HOME.master" AutoEventWireup="true" CodeFile="UnpostedList.aspx.cs"
    Inherits="USGAAP_BILLING_UnpostedList" Title="Untitled Page" %>
        ...
        function checkDec1(el) {
            ...
            if (ex.test(el.value) == false) {
                ...
                if (el.value == '') {
                    ...
                    var lblAmountInr = row.cells[20].innerHTML;
                    var pendingpostedValue = row.cells[22].innerHTML;                      
                    var postingValue = row.cells[23].getElementsByTagName("input")[0].value;  
                                        
                    if(pendingpostedValue=='0.00' || pendingpostedValue=='' || pendingpostedValue == '0')
                    {
                        unpostedValue = Math.round((parseFloat(lblAmountInr) - parseFloat(postingValue)) * 100) / 100;                                                                                                    
                        row.cells[25].getElementsByTagName("input")[0].value = unpostedValue;
                                            
                        if (postingValue == '' || postingValue == '0.00'|| postingValue == '0') {
                            row.cells[25].getElementsByTagName("input")[0].value = '';
                        }
                                                                
                        if (parseFloat(lblAmountInr) < parseFloat(postingValue)) {
                            row.cells[23].getElementsByTagName("input")[0].style.borderColor = "#F7627F";
                            document.getElementById('<%=btnPost.ClientID %>').visibility='hidden';
                            return true;
                        }
                        else {
                            row.cells[23].getElementsByTagName("input")[0].style.borderColor = "";
                            return false;
                        }
                    }
                    else
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder2" runat="Server">
    ...
    <div align="center" style="margin-top: 20px;">
        <fieldset style="width: 90%">
            <legend style="text-align: center;">Unposted List</legend>
            <table width="100%">
                <tr>
                    <td>
                        Start Date:
            ...
            <div style='overflow: auto; width: 100%; height: 450px; border: 1px solid lightgray;'>
                <asp:GridView ID="gvPostingList" runat="server" AutoGenerateColumns="False" CellPadding="4"
                    ...
                    <Columns>
                        ...
                        <asp:BoundField DataField="DESCRIPT" HeaderText="BUS_SEGMENT_DESC" />
                        <asp:BoundField DataField="GLCODE" HeaderText="GLCODE" />
                        <asp:BoundField DataField="GL_DESCRIPT" HeaderText="GLCODE_DESC" />
                        <asp:BoundField DataField="PL_BS" HeaderText="GLCODE_TYPE" />
                        <asp:TemplateField HeaderText="END_MARKET">
                            <ItemTemplate>
                                ...
                                <asp:Label ID="lblBusinessSegmentDesc" runat="server" Visible="false" Text='<%# Eval("DESCRIPT") %>' />
                                <asp:Label ID="lblGlcode" runat="server" Visible="false" Text='<%# Eval("GLCODE") %>' />
                                <asp:Label ID="lblGlcodeDesc" runat="server" Visible="false" Text='<%# Eval("GL_DESCRIPT") %>' />
                                <asp:Label ID="lblGlcodeType" runat="server" Visible="false" Text='<%# Eval("PL_BS") %>' />
                        ...
                        <asp:TemplateField HeaderText="POSTING_MONTH">
                            <ItemTemplate>
                                <asp:DropDownList ID="ddlPostingMonth" runat="server" Width="100%" Enabled="false">
